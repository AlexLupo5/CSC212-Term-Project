...
void Convex::computeConvex(){
  ...
  for (int i = 1; i < this->Point.size(); i++){
    if (this->Point[i].y < this->Point[small].y || (this->Point[i].y 
      < this->Point[small].y && this->Point[i].x < this->Point[small].x)){
        small = i;
    }
  }

  //Swap the lowest and first point
  std::swap(this->Point[0], this->Point[small]);

  //Sort the other point based on polar angle
  temp = this->Point[0];
  std::sort(this->Point.begin() + 1, this->Point.end(), compare);

  //Create the convex hull with the three first points
  ConHull.push_back(this->Point[0]);
  ConHull.push_back(this->Point[1]);
  ConHull.push_back(this->Point[2]);

  //Compute the rest of the points
  for (int i = 3; i < this->Point.size(); i++){
...
bool Convex::compare(Point a, Point b){
      //Check orientation
      int check = temp.orientation(temp, a, b);
